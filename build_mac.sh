...
if [ ! -f "qt-everywhere-opensource-src-5.8.0.tar.gz" ]; then
  echo "Getting source files..."
  wget -N https://download.qt.io/official_releases/qt/5.8/5.8.0/single/qt-everywhere-opensource-src-5.8.0.tar.gz
  gunzip < qt-everywhere-opensource-src-5.8.0.tar.gz > qt-everywhere-opensource-src-5.8.0.tar
else
  echo "Already downloaded source files."
fi

if [ -f "qt-everywhere-opensource-src-5.8.0.tar" ]; then
  echo "Extracting downloaded source files..."
  rm -rf qt-everywhere-opensource-src-5.8.0/
  tar xf qt-everywhere-opensource-src-5.8.0.tar
fi
